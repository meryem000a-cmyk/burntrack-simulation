...
\begin{titlepage}
  ...
  \begin{minipage}{0.48\textwidth}
    \begin{flushleft}
      \includegraphics[height=3.0cm]{CENTRALE_CASA_LOGO_OK.png}
    \end{flushleft}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\textwidth}
  ...
  \begin{center}
    \textsc{\LARGE École Centrale Casablanca}\\[0.6cm]
    \textsc{\LARGE PROJET LEARNING BY DOING}\\[0.3cm]
    \textsc{\large Année universitaire 2025--2026}\\[1.0cm]
...
\paragraph{Caveat matériel.} La table~\ref{tab:bom} présente la liste des composants retenus pour le rover et leur justification. À ce stade du projet, seuls le schéma Fritzing (\texttt{circuit/}) et les scripts de déploiement génériques pour Raspberry Pi sont versionnés dans le dépôt. Les pilotes spécifiques (LoRa/CBOR, MPU6050, ADS1015, NEO-6M, etc.), le firmware embarqué et les fichiers de fabrication 3D n'y figurent pas encore. La communication décrite en \S~4.5.8 est donc une architecture cible validée par le schéma électrique, et non une pile logicielle opérationnelle testée en conditions réelles.
