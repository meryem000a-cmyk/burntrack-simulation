...
Le système repose sur quatre piliers : (1) un moteur Rothermel v3 corrigé, implémentant 50 modèles de combustible dont 28 spécifiques à l'Afrique~; (2) un automate cellulaire stochastique couplé à un correcteur par apprentissage automatique (MLP, $R^2 = 0{,}987$, MAE $= 0{,}735$ m/min sur le dataset de littérature de 1\,840 observations terrain, à mettre en perspective avec $R^2 = 0{,}12$ sur les données satellites brutes FIRMS)~; (3) un rover 6WD/6WS à base de Raspberry Pi 4, équipé de capteurs environnementaux, d'un anémomètre DIY et d'une caméra pour la classification botanique par vision artificielle (YOLO + CNN)~; (4) une communication LoRa entre le rover et la station de base, couplée à une visualisation en temps réel par WebSocket. Le système complet a été validé spatialement sur deux feux réels majeurs en utilisant des produits Landsat 8/9 fusionnés par seuillage dNBR (constituant une vérité terrain dérivée et non une vérité terrain directe) : sur l'incendie de Table Mountain (avril 2021) avec un IoU de 0,07 et une précision de 0,93 (seuil dNBR 0,40), et sur l'incendie de Knysna (juin 2017) où la simulation atteint un IoU strict de 0,516 (IoU généreux de 0,620) et un score F1 strict de 0,681 (F1 généreux de 0,765) au seuil optimal de 0,05.
...
The system relies on four pillars: (1) a corrected Rothermel v3 engine, implementing 50 fuel models including 28 specific to Africa; (2) a stochastic cellular automaton coupled with a machine learning corrector (MLP, $R^2 = 0.987$, MAE $= 0.735$ m/min on the literature dataset of 1,840 field observations, paired with $R^2 = 0.12$ on raw FIRMS satellite data); (3) a Raspberry Pi 4-based 6WD/6WS rover, equipped with environmental sensors, a DIY anemometer, and a camera for botanical classification via computer vision (YOLO + CNN); (4) a LoRa communication link between the rover and the base station, coupled with real-time WebSocket visualization. The full system was validated spatially on two major real wildfires using Landsat 8/9 products fused via dNBR thresholding (constituting a derived ground truth rather than direct field truth): on the Table Mountain wildfire (April 2021) achieving an IoU of 0.07 and precision of 0.93 (dNBR threshold 0.40), and on the Knysna wildfire (June 2017) achieving a strict IoU of 0.516 (generous IoU of 0.620) and strict F1 score of 0.681 (generous F1 of 0.765) at the optimal threshold of 0.05.
